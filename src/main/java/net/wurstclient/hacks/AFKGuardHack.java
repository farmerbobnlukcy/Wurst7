...
public final class AFKGuardHack extends Hack implements TitleScreenListener,
	ChatInputListener, UpdateListener, LeftClickListener, RightClickListener
{
	...
	// Phrase lists
	private static final String[] ENTER_AFK =
		new String[]{"you are now afk", "now afk", "[afk]", "afk mode"};
	private static final String[] EXIT_AFK =
		new String[]{"you are no longer afk", "no longer afk",
			"you are no longer idle", "no longer idle"};
	...
	public void onEnable()
	{
		...
		EVENTS.add(TitleScreenListener.class, this); // remove if you don't use
														// titles
	...
	{
		String chatlines = event.getChatLines() != null
			? event.getChatLines().toString() : null;
		if(!listenChat || chatlines == null)
			return;
	...
	private void disableAllCombatHacks()
	{
		for(Hack h : WurstClient.INSTANCE.getHax().getAllHax())
		{
			if(h == this)
				continue;
			if(h.getCategory() != Category.COMBAT)
				continue;
			if(!h.isEnabled())
				continue;
	...
	private static void tryDisable(Hack h)
	{
		...
		}catch(Throwable t1)
		{
			try
			{
				System.out.println("Error disabling hack: " + h.getName());
			}catch(Throwable t2)
			{ /* last resort */ }
		}
	}
	...
	private static void tryEnable(Hack h)
	{
		...
		}catch(Throwable t1)
		{
			try
			{
				System.out.println("Error enabling hack: " + h.getName());
			}catch(Throwable t2)
			{ /* last resort */ }
		}
	}
