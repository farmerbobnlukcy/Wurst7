...
public final class ItemEspHack extends Hack implements UpdateListener,
	...
	private final ArrayList<net.minecraft.entity.ExperienceOrbEntity> expOrbs = new ArrayList<>();
	...
	public void onUpdate() {
		...
		if (highlightExpOrbs.isChecked()) {
			StreamSupport.stream(MC.world.getEntities().spliterator(), false)
					.filter(e -> e instanceof net.minecraft.entity.ExperienceOrbEntity)
	...
	private ItemEntity findClosestItem() {
		...
		// If exp orbs are enabled, check if any are closer than the closest item
		if (highlightExpOrbs.isChecked() && !expOrbs.isEmpty()) {
			for (net.minecraft.entity.ExperienceOrbEntity orb : expOrbs) {
				...
				if (distSq < closestDistSq) {
					// Return the item we found so far, since we can't return an exp orb here
					return closest;
				}
			}
		}
	...
	public void onRender(MatrixStack matrixStack, float partialTicks) {
		...
		if (style.hasLines()) {
			...
			if (!expOrbs.isEmpty() && highlightExpOrbs.isChecked()) {
				...
				for (net.minecraft.entity.ExperienceOrbEntity e : expOrbs) {
					Vec3d center = EntityUtils.getLerpedBox(e, partialTicks).getCenter();
				...
				if (!expOrbEnds.isEmpty()) {
					RenderUtils.drawTracers(matrixStack, partialTicks, expOrbEnds,
							expOrbLineColor, false);
				}
			}
